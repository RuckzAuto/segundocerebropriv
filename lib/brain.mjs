...
FORMATAÇÃO DAS RESPOSTAS (WhatsApp/Telegram):
- Use *um asterisco* para negrito. NUNCA use **dois asteriscos**, isso não funciona no WhatsApp/Telegram e aparece errado.
- Use _sublinhado_ para itálico quando fizer sentido.
- Ao listar mais de um item (notas, tarefas, links), separe cada item com uma linha em branco — nunca deixe tudo grudado num parágrafo só.
- Use emojis com moderação pra facilitar a leitura (ex: 📌 nota, ✅ confirmação, 📁 pasta, ⚠️ aviso, 🔗 link) — só onde ajudar, sem exagerar.
- Sempre que citar o nome de uma pasta, deixe em *negrito*.`;
  ...
  try {
    ...
    if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
      ...
      if (callName === "salvar_nota") {
        ...
        if (rs.rows.length === 0) {
          aiReply = `⚠️ A pasta *${args.pasta_path}* não existe.`;
        } else {
...
Formate a resposta pro WhatsApp/Telegram: use *um asterisco* pra negrito (NUNCA **dois asteriscos**, não funciona nessas plataformas), separe cada nota/item diferente com uma linha em branco (nunca tudo grudado num parágrafo só), e use emoji com moderação (📌 nota, 🔗 link, ✅ tarefa feita, ⚠️ aviso) só onde ajudar a ler.`;
        ...
        if (rs.rows.length === 0) {
          aiReply = `⚠️ A pasta *${pasta_path}* não foi encontrada.`;
        } else {
          const folderId = rs.rows[0].id;
          const notesRs = await db.execute({
          ...
          if (matchingNotes.length === 0) {
            ...
            if (callName === "editar_nota") {
              ...
              aiReply = `✏️ Nota atualizada em *${pasta_path}*${msgExtra}`;
            } else {
              await db.execute({
                sql: 'DELETE FROM notes WHERE id = ?',
                args: [targetNote.id]
              });
              aiReply = `🗑️ Nota removida de *${pasta_path}*${msgExtra}`;
            }
          }
        }
        ...
        if (rs.rows.length === 0) {
          ...
          if (subRs.rows[0].count > 0) {
            aiReply = `⚠️ Não é possível remover a pasta *${args.pasta_path}* pois ela contém subpastas.`;
          } else {
            ...
            aiReply = `🗑️ Pasta *${args.pasta_path}* e suas notas foram removidas.`;
          }
        }
        ...
        if (rs.rows.length === 0) {
          ...
          const linhas = rs.rows.map(row => `📁 *${row.path}* — ${row.qtd} nota${row.qtd === 1 ? '' : 's'}`);
          aiReply = `📂 *Visão geral*\n\n${linhas.join('\n\n')}`;
        }
    ...
    // Rede de seguranca: WhatsApp/Telegram usam *um asterisco* pra negrito, nao **dois** (Markdown padrao).
    // A IA e instruida a nao usar **, mas isso normaliza qualquer deslize sem depender 100% do modelo.
