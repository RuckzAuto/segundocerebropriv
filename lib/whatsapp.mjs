...
export async function connectWhatsapp() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      const debugNumber = normalizeJid(msg.key.remoteJid);
      console.log(`[WhatsApp][DEBUG] mensagem recebida — remoteJid=${msg.key.remoteJid} numeroExtraido=${debugNumber} fromMe=${msg.key.fromMe} allowedNumber=${allowedNumber} temTexto=${!!(msg.message?.conversation || msg.message?.extendedTextMessage?.text)}`);
